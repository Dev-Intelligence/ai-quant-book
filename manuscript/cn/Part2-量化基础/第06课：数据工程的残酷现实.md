...
> | 阶段 | 推荐数据源 | 说明 |
> |------|-----------|------|
> | 学习/原型 | Yahoo Finance (yfinance) | 免费日线，够用 |
> | 小规模实盘 | Polygon.io / Alpaca | 实时行情，API 友好 |
> | 机构级生产 | Databento / 交易所直连 | L1/L2 实时数据，低延迟 |
>
...
> **生产提示：标准化数据模型**
> 不同市场和数据源使用不同的代码格式（`600519.SH`、`AAPL`、`0700.HK`、`BTC/USDT`）。生产系统需要一个**统一的规范化层**（Normalizer），将外部格式转换为内部标准格式（如 `SYMBOL.EXCHANGE`：`AAPL.NASDAQ`、`0700.HKEX`）。这是实现多市场交易的前提条件。同时注意符号解析的常见陷阱：退市股票静默失败、公司行为导致代码变更、多段式代码（如 BRK.B）等。详见第 19 课。
